...
def _hook_claude_code(event: dict) -> int:
    ...
    reason = _format_reason(decision, action, target)
    if verdict == "block":
        perm = "deny"
    elif verdict == "ask":
        perm = "ask"
...
# Verdict → human-facing lead + the NIST AI RMF response function it embodies.
_VERDICT_LEAD = {
    "block": "⛔ Elder Mind blocked",
    "ask": "🟠 Elder Mind needs your approval",
    "warn": "⚠ Elder Mind flagged",
}
_VERDICT_NIST = {"block": "MANAGE", "ask": "MANAGE", "warn": "MEASURE"}


def _format_reason(decision: dict, action: str = "", target: str = "") -> str:
    """Structured, human-facing block message — mirrors the README/demo so what a
    user sees in their IDE matches what's advertised. Leads with the consequence,
    maps to OWASP/NIST, cites the decision id + audit path, and (for ask/block)
    always shows the escape hatch so a false positive never dead-ends the user."""
    verdict = decision.get("verdict", "")
    risk = decision.get("risk") or {}
    score, tier = risk.get("score"), risk.get("tier")
    asi = decision.get("asi")
    asi_name = _ASI_NAMES.get(asi, "") if asi else ""
    nist = _VERDICT_NIST.get(verdict)
    preview = decision.get("preview")
    did = decision.get("decision_id")

    head = _VERDICT_LEAD.get(verdict, "Elder Mind")
    if action or target:
        head += f": {action}({target})" if action else f": {target}"
    lines = [head]
    bits = []
    if score is not None:
        bits.append(f"risk {score}/25 ({tier})")
    if asi:
        bits.append(f"OWASP {asi}{(' ' + asi_name) if asi_name else ''}")
    if nist:
        bits.append(f"NIST RMF: {nist}")
    if bits:
        lines.append("   " + " · ".join(bits))
    if preview:
        lines.append(f"   ⚠ {preview}")
    if did:
        lines.append(f"   decision {did} · logged to .eldermind/audit.jsonl")
    if verdict in ("ask", "block"):
        lines.append("   to allow: add a rule to .eldermind/policy.yaml")
    return "\n".join(lines)
