...
Other subcommands:
    eldermind init <tool>          # guided install (claude-code|opencode|kiro|cursor)
    eldermind install <tool>       # non-interactive install
    eldermind hook <tool>          # per-adapter hook target
    eldermind scan <cmd|lockfile>  # supply-chain check (OSV)
    eldermind explain <id>         # reconstruct a past decision from the audit log
    eldermind verify               # check the audit hash-chain is intact
    eldermind summary              # audit aggregate (NIST MEASURE)
    eldermind pin <name> <json>    # pin a tool/MCP descriptor; detect drift (TOFU)
