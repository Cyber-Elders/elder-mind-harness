...
# --------------------------------------------------------------------------
# OSV response PARSING — exercise the real parser (transport mocked, offline).
# Previously only the _query_osv boundary was mocked; this covers the actual
# malicious/vulnerable/clean classification + the deps.dev date math.
# --------------------------------------------------------------------------
class _FakeResp:
...
def _patch_urlopen(monkeypatch, payload):
    monkeypatch.setattr(supplychain.urllib.request, "urlopen",
                        lambda req, timeout=0: _FakeResp(payload))


def test_query_osv_parses_malicious(monkeypatch):
    _patch_urlopen(monkeypatch, {"vulns": [{"id": "MAL-2024-0001"}]})
    r = supplychain._query_osv(supplychain.Package("PyPI", "evilpkg", "1.0.0"))
    assert r is not None and r.status == "malicious" and r.source == "osv-api"


def test_query_osv_parses_vulnerable(monkeypatch):
    _patch_urlopen(monkeypatch, {"vulns": [{"id": "GHSA-aaaa-bbbb-cccc"}]})
    assert supplychain._query_osv(supplychain.Package("npm", "foo", "1.0.0")).status == "vulnerable"


def test_query_osv_parses_clean(monkeypatch):
    _patch_urlopen(monkeypatch, {"vulns": []})
    assert supplychain._query_osv(supplychain.Package("npm", "lodash", "4.17.21")).status == "clean"


def test_query_osv_network_error_returns_none(monkeypatch):
    def boom(req, timeout=0):
        raise supplychain.urllib.error.URLError("offline")
    monkeypatch.setattr(supplychain.urllib.request, "urlopen", boom)
    assert supplychain._query_osv(supplychain.Package("npm", "x", "1.0.0")) is None


def test_release_age_days_parses_published_date(monkeypatch):
    _patch_urlopen(monkeypatch, {"publishedAt": "2020-01-01T00:00:00Z"})
    age = supplychain.release_age_days(supplychain.Package("npm", "old-pkg", "1.0.0"))
    assert age is not None and age > 1000  # published 2020 -> years old


def test_release_age_days_unpinned_returns_none():
    assert supplychain.release_age_days(supplychain.Package("npm", "x", None)) is None


# --------------------------------------------------------------------------
# Detector severity mapping — keep it honest: no current pattern forces 'ask'.
# --------------------------------------------------------------------------
def test_detector_severity_mapping():
    assert detectors.SEVERITY_VERDICT == {
        "critical": "ask", "high": "warn", "medium": "warn", "low": "allow",
    }


def test_no_current_detector_is_critical():
    # Documents the current ceiling: detectors only ever surface/'warn' today.
    # The 'critical -> ask' rung is reserved for future patterns (module docstring).
    severities = {p["severity"] for p in detectors._PATTERNS}
