...
# --------------------------------------------------------------------------
# Fail-safe — a policy that cannot load must fall safe to "ask" (never allow)
# --------------------------------------------------------------------------
def test_corrupt_policy_fails_safe_to_ask(tmp_path):
    bad = tmp_path / "policy.yaml"
    # structurally valid YAML, but an unknown action -> load_policy raises PolicyError
    bad.write_text(
        'version: "0.1"\n'
    ...
    # even a benign action must ask when the policy can't load (fail-safe, not fail-open)
    d = decide("bash", "ls -la", policy=bad)
    assert d.verdict == "ask"
    assert "failing safe" in d.reason
    assert d.policy_version == "error"
    assert d.exit_code() == 2  # stop-and-surface


def test_missing_policy_file_fails_safe_to_ask(tmp_path):
    d = decide("bash", "ls -la", policy=tmp_path / "does-not-exist.yaml")
    assert d.verdict == "ask"
    assert "failing safe" in d.reason


def test_policy_with_non_ascii_loads(tmp_path):
    # A user policy containing non-ASCII (accents / arrows / symbols) must load on
    # EVERY OS. Guards the explicit UTF-8 read in policy.py: on a Windows cp1252
    # locale a bare read_text() would raise UnicodeDecodeError — which decide()
    # does NOT catch (it only catches PolicyError), i.e. a hard crash.
    ...
    pol.write_text(
        ...
        "rules:\n"
        "  - id: café-rule\n    asi: ASI02\n    impact: 5\n    likelihood: 5\n"
        '    action: block\n    description: "déjà-vu → blocked ☠"\n'
        "    match: {tool: [bash], pattern: 'rm -rf /'}\n",
        encoding="utf-8",
    )
    d = decide("bash", "rm -rf /", policy=pol)
    assert d.verdict == "block"
    assert d.rule_id == "café-rule"
